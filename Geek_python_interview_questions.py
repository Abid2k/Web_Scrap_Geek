    ...
    try:
        ...
        while header:
            for sibling in header.find_next_siblings():
                if sibling.name == 'h2': 
                    header =sibling# Stop if another section starts
                    break
                if sibling.name == 'h3':  # Extract question from <h3> tags
                    ...
                    print("/n")
                    # print(f'Extracted answers {questions_answers}')
                    
                # If we find a new section, move to that section's header
            if header and header.name == 'h2' and header.find_next_sibling() and header.find_next_sibling().name == 'h2':
                header = header.find_next_sibling()

        print (f'Length len: {len(questions_answers)}')
